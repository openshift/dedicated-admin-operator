...
define create_push_catalog_image
	...
	if [[ "$$(echo $(4) | tr [:upper:] [:lower:])" == "true" ]]; then \
		deployed_hash=$$(curl -s 'https://gitlab.cee.redhat.com/$(5)/raw/master/$(6)' | docker run --rm -i quay.io/app-sre/yq yq r - 'resourceTemplates[*].targets(namespace.$ref==/services/osd-operators/namespaces/hivep01ue1/cluster-scope.yml).ref') ;\
		delete=false ;\
